...
export function classifyError(error: Error | string, status?: number): ErrorCategory {
  ...
  if (
    ...
    (status && (status === 400 || status === 401 || status === 403 || status === 404))
  ) {
...
export function estimateNetworkCondition(recentErrors: ErrorCategory[] = []): NetworkCondition {
  const timeoutCount = recentErrors.filter((e) => e === ErrorCategory.TIMEOUT).length
  const temporaryErrorCount = recentErrors.filter((e) => e === ErrorCategory.TEMPORARY).length
...
export function shouldRetry(
  ...
  if (errorCategory === ErrorCategory.PERMANENT) {
    return {
      retry: false,
      delay: 0,
      reason: `Error is permanent: ${typeof error === 'string' ? error : error.message}`,
    }
  }
