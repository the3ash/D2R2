...
class NotificationManager {
  ...
  public addNotification(
    ...
    if (type === 'loading' && title === TOAST_STATUS.DROPPING) {
      ...
      if (this.activeLoadingId) {
        return this.updateNotification(this.activeLoadingId, title, message, type, {
          imageUrl,
          showSystem,
          showPage,
          direction,
      ...
      if (loadingNotifications.length > 0) {
        loadingNotifications.forEach((item) => {
          console.log(`Marking existing loading notification as processed: ${item.id}`)
          item.processed = true
        })
      }
    ...
    if (similarNotification) {
      console.log(`Similar notification already in queue: ${id}, type: ${type}`)
   ...
   */
  private async displayNotification(notification: NotificationItem): Promise<void> {
    const { id, title, message, type, imageUrl, showSystem, showPage, direction } = notification

    try {
      // Display page notification
      if (showPage) {
        await this.displayPageToast(id, title, message, type, direction, imageUrl)
      }
  ...
  private async displayPageToast(
    ...
    try {
      ...
      if (
        ...
        console.log(`[Toast][${id}] Cannot show toast on special page: ${activeTab.url}`)
        return
      }
    ...
    return new Promise<void>((resolve) => {
      try {
        ...
        chrome.permissions.contains({ permissions: ['notifications'] }, (hasPermission) => {
          if (!hasPermission) {
            console.error(`[System][${id}] No notification permission, cannot show notification`)
            resolve()
            return
          }
          ...
          const handleNotificationClick = (clickedId: string) => {
            if (clickedId === id && imageUrl) {
              console.log(`[System][${id}] Notification clicked, opening URL: ${imageUrl}`)
  ...
  public cleanupOldNotifications(): void {
    ...
    this.queue = this.queue.filter((item) => !item.processed || now - item.timestamp < MAX_AGE_MS)

    console.log(`Cleaned up old notifications, queue length: ${this.queue.length}`)
  }
}
...
export function showProcessingNotification(
  ...
  if (activeId) {
    ...
    return manager.updateNotification(activeId, TOAST_STATUS.DROPPING, message, 'loading', {
      showSystem: true,
      showPage: true,
      direction: 'top',
      forceShow: true,
...
export function showNotification(
  ...
  if (type === 'success' || type === 'error' || type === 'info' || type === 'loading') {
