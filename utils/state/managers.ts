...
export class ExtensionStateManager {
  ...
  public startInitialization(source: string): boolean {
    ...
    if (this.isInitializing) {
      console.log(`${source}: Another initialization is in progress, waiting...`)
      return false
    }
    ...
    if (now - this.lastInitTime < this.MIN_INIT_INTERVAL) {
      console.log(`${source}: Last initialization was ${now - this.lastInitTime}ms ago, too recent`)
      return false
    }
    ...
    console.log(`${source}: Beginning initialization... (attempt ${this.initializationAttempts})`)
...
export class UploadTaskManager {
  ...
  public createTask(info: chrome.contextMenus.OnClickData, tabOrFolder?: chrome.tabs.Tab | string | null): string {
    ...
    console.log(`[UploadTask] Created task ${taskId} with state ${UploadState.PENDING}`)
    return taskId
  }

  public updateTaskState(taskId: string, state: UploadState, errorMsg?: string) {
...
export class PageStateManager {
  ...
  public addPendingMenuClick(info: chrome.contextMenus.OnClickData, tab?: chrome.tabs.Tab) {
    this.pendingMenuClicks.push({
      info,
      tab,
      timestamp: Date.now(),
    })
    console.log(`Added pending menu click, total: ${this.pendingMenuClicks.length}`)
  }
