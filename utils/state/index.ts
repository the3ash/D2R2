...
/**
 * Page state manager for tracking page/tab state
 */
...
class UploadTaskManager {
  ...
  readonly MENU_CLICK_THROTTLE: number = 500; // ms

  private lastMenuClickTime: number = 0;
  private menuClickHandler: any = null;
  private menuClickPending: boolean = false;
  ...
  public canProcessMenuClick(): boolean {
    ...
    // 总是允许处理菜单点击
    this.lastMenuClickTime = Date.now();
    console.log("Menu click processing allowed");
    return true;
  }
  ...
  public getRetryCount(taskId: string): number {
    return this.tasks.get(taskId)?.retryCount || 0;
  }

  public getTaskElapsedTime(taskId: string): number {
    const task = this.tasks.get(taskId);
    if (!task) return 0;
